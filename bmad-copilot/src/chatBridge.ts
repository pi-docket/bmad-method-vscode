/**
 * @fileoverview Chat Bridge — Official BMAD GitHub Copilot Prompt Executor
 *
...
⚠️ **GitHub Copilot prompt files not found**
 ...
 * | `/run <cmd>` | Prompt Executor    | Read prompt file → resolve refs → LLM     |
 ...
 * | *(free text)*| Fuzzy → Executor   | Match to command or BMAD context chat      |
 */
export class ChatBridge {
  ...
  private handleInstall(stream: vscode.ChatResponseStream): vscode.ChatResult {
    ...
    stream.markdown(
      '🚀 **Opening terminal to run BMAD installer…**\n\n' +
  ...
  /**
   * Handle `/status` — show BMAD installation health.
   *
  ...
  /* -------------------------------------------------------------- */
  /*  /help — List commands and guidance                             */
  /* -------------------------------------------------------------- */
    ...
    if (state.helpEntries.length > 0) {
      ...
      for (const [phase, entries] of byPhase) {
        ...
        for (const h of entries) {
          const cmd = h.command ? `\`/run ${h.command}\`` : `*${h.code || '—'}*`;
  ...
  /* -------------------------------------------------------------- */
  /*  /run — Prompt File Executor (core)                            */
  /* -------------------------------------------------------------- */

  /**
   * Handle `/run <command-name>` — execute a BMAD command.
   *
   * **Execution strategy:**
  ...
  private handleAgents(
    ...
    for (const [mod, group] of byModule) {
      ...
      for (const a of group) {
        ...
        lines.push(
          `| ${a.name} | ${a.displayName || '—'} | ${a.icon || ''} ${a.title || ''} | \`/run ${slashName}\` |`,
        );
      }
      lines.push('');
    }
  ...
  private handleWorkflows(
    ...
    if (state.helpEntries.length > 0) {
      ...
      for (const [phase, entries] of byPhase) {
        ...
        lines.push('|---|---|---|---|---|');
        for (const h of entries) {
          const cmd = h.command ? `\`/run ${h.command}\`` : '*—*';
  ...
  private async executeCommand(
    ...
    stream.markdown(
      `🚀 **Executing**: \`${command.slashName}\`  →  CLI: \`${command.cliSyntax}\`\n` +
        `> ${command.description}\n\n---\n\n`,
    );
  ...
  private buildFromPromptFile(
    ...
    // Compose final prompt — body as-is + optional user input
    if (userInput) {
  ...
  private handleCommandNotFound(
    ...
    if (suggestions.length > 0) {
      const list = suggestions
        .map((s) => `- \`/run ${s.slashName}\` — ${s.description}`)
        .join('\n');
      stream.markdown(
        `❓ Command \`${rawName}\` not found.\n\nDid you mean:\n${list}`,
      );
    } else {
      stream.markdown(
        `❓ Command \`${rawName}\` not found.\n\n` +
