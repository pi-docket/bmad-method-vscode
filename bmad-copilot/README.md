...
- `.github/agents/` — Copilot agent files (`.agent.md`)
...
2. Verify `.github/prompts/` exists (warns if missing)
...
code --install-extension releases/bmad-copilot-adapter-0.2.0.vsix
```
...
| `update`    | Rescan `.github/prompts/`, count manifests, write sentinel for VS Code |
...
1. **Ensure `.github/prompts/` exists** and contains `bmad*.prompt.md` files.
...
`.github/prompts/` and `_bmad/_config/` CSV manifests. If prompt files
